...
# OMR
OMR.act <- read.csv(file.path(action_path,'alt1_status-quo_OMR_input.csv'),header=T)
...
# X2
X2.dist.act <- read.csv(file.path(action_path,'alt1_status-quo_x2_input.csv'),header=T)
X2.dist.act <- X2.dist.act[which(X2.dist.act==paste0(action)),]
rownames(X2.dist.act) <- X2.dist.act[,2]
X2.target <- X2.dist.act[,3:14]

# Prey
PD.act <- read.csv(file.path(action_path,'alt1_status-quo_zoop_input.csv'),header=T)
