...
# Secchi
#secchi.act_test <- read.table(file.path(action_path,'demo_action_inputs/AWC_Yolo_Secchi.txt'),header=F)
secchi.act <- read.table(file.path(action_path,'AWC_SAVtreatment_8.5.2_Secchi.txt'),header=F)
for (y in 1:(n.years+1)) {
for (m in 1:12) {
...
#Time difference of 20.14476 mins 
parallel_runtime


#saveRDS(outzp,file.path(output_path,'alt_MaxWater_NF_AquaticWeed_adjHist.rds'))
#saveRDS(outzp,file.path(output_path,'alt_MaxDS_Even_NF_AquaticWeed_adjHist.rds'))
#saveRDS(outzp,file.path(output_path,'alt_MaxDS_Even_NF_AquaticWeed_2022MED.rds'))
saveRDS(outzp,file.path(output_path,'alt_MaxWater_NF_AquaticWeed_2022MED.rds'))
